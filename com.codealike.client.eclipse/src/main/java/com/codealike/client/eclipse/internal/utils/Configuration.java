...
import com.codealike.client.eclipse.internal.model.GlobalSettings;
import com.codealike.client.eclipse.internal.model.PluginSettings;
...
public class Configuration {
	...
	/*
	 * loadCodealikeSettings: This method loads user settings stored in codealike
	 * user folder After this method call userToken and user profile information
	 * should been loaded
	 */
	public void loadGlobalSettings() {
		...
		try {
			if (Files.exists(codealikeSettingsFile.toPath())) {
				...
				GlobalSettings existingConfiguration = mapper.readValue(new FileInputStream(codealikeSettingsFile),
						GlobalSettings.class);

				if (existingConfiguration != null) {
					this.globalSettings.setUserToken(existingConfiguration.getUserToken());
					this.globalSettings.setApiUrl(Optional.ofNullable(existingConfiguration.getApiUrl())
							.orElse("https://codealike.com/api/v2"));
				} else {
	...
	/*
	 * saveCurrentGlobalSettings This method saves current running instance settings
	...
	/*
	 * saveCodealikeGlobalSettings This method saves user settings configured in
	 * current configuration instance to the codealike user folder
	 */
