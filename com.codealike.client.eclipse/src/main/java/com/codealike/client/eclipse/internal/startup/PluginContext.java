...
public class PluginContext {
	...
	public PluginContext() {
		...
		this.dateTimeFormatter = new DateTimeFormatterBuilder().appendYear(4, 4).appendLiteral("-").appendMonthOfYear(2)
				.appendLiteral("-").appendDayOfMonth(2).appendLiteral("T").appendHourOfDay(2).appendLiteral(":")
	...
	private UUID tryGetLegacySolutionIdV1(IProject project) {
		...
		try {
			...
			if (projectNode != null) {
				// if projectId is not created yet, try to create a unique new one and register
				// it.
